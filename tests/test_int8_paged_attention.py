...
def create_random_block_tables(
    ...
    """Non-contiguous logical->physical block table [B, max_blocks]."""
...
def build_slot_mapping(
    ...
    """logical token -> physical slot for cache write."""
...
def torch_bf16_attention_reference(
    ...
    """BF16 decode ref: query [B,Hq,D], keys/values lists [T,Hkv,D]."""
    B, Hq, D = query.shape
    Hkv = keys[0].shape[1]
    group = Hq // Hkv
    outputs = []
...
def test_int8_paged_attention_v4_matches_reference():
    ...
    for name, out in [("v3", out_v3), ("v4", out_v4), ("v4-split", out_v4s)]:
        ...
        # bf16 tl.dot may be slightly looser than fp32 v3
