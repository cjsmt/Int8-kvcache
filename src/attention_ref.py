...
def decode_attention_ref(q, k, v):
    """q: [B, Hq, D] k: [B, Hkv, T, D] v: [B, Hkv, T, D] output: [B, Hq, D]"""
    B, Hq, D = q.shape
...
def decode_attention_int8_dynamic(q, k, v):
    """这是“正确性 reference”，不是最终高性能 kernel。 做法： 1. q per-head 动态量化 2. k/v per-head 动态量化 3. 反量化后走 reference attention 用来证明量化误差是否可接受。"""
