"""
vllm_int8_cache_ops.py

INT8 KV Cache runtime write path for vLLM 0.26.x.
...
Quantization:
    symmetric INT8
    per-token-per-head dynamic scale
"""
...
def _reshape_and_cache_int8_kernel(
    ...
    # vLLM can use negative slots for padding / skipped tokens.
    valid_slot = slot >= 0
    ...
    # round-to-nearest (half away from zero); tl.rint unavailable in Triton 3.6
...
def reshape_and_cache_int8(
    ...
    """
    Quantize new K/V and write them into INT8 paged KV cache.
    ...
    # --------------------------------------------------------
    # contiguous slot mapping makes addressing simpler
    # --------------------------------------------------------
...
def allocate_int8_kv_cache(
    ...
    """
    Allocate the four tensors needed by our INT8 KV cache.

    Returns
    -------
    key_cache:
        INT8 [num_blocks, BS, Hkv, D]

    value_cache:
        INT8 [num_blocks, BS, Hkv, D]

    k_scale_cache:
        FP32 [num_blocks, BS, Hkv]
